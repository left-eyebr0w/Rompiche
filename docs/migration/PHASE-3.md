...
> **Valeurs résolues** : `h = clamp(0.5·overlap, 0.3, +∞)` m · `debounce = 150 ms` · `FONDU = 0.02 s` (20 ms) · évaluation LOD à `30 Hz` · `PAS` d'ajustement `r1` = `0.5 m` · `R1_MIN = 1.0 m` · seuils budget `busyHi = 0.95`, `busyLo = 0.50` · tick budget `1 Hz`.

**T-3.1 — Utilitaire crossfade** · `ds/ui_kits/diorama/coords.js` (ou `lod.js` new)
- *Action* : `export function fondu(t)` → `[g_bas, g_haut]` puissance constante (§3). `t` clampé [0,1].
- *Dépend* : —
- *Test* : `fondu(0)=[1,0]`, `fondu(1)=[0,1]`, `g_bas²+g_haut² ≈ 1` pour `t=0.5`.

**T-3.2 — `LodController`** · `ds/ui_kits/diorama/LodController.js` (new)
- *Action* : suit les sources proches des frontières ; `évaluerLod(s, params, now, rec)` avec hystérésis + anti-rebond (§4). `LodParams` dérivés des `bands` : `h = clamp(0.5·overlap,0.3,∞)`. Callbacks `onDémote(s, de, vers)` / `onPromote(s, de, vers)` fournis par `RainSampler`.
- *Signatures* : `export class LodController { constructor(params, hooks); track(source); évaluerLod(...); setParams(params) }`.
- *Dépend* : T-0.B2
- *Test* : une source oscillant dans `[r1−h, r1+h]` ne bascule pas ; au-delà de `r1+h` → un seul `lod` L1→L2.

**T-3.3 — Câblage démotion/promotion** · `ds/ui_kits/diorama/RainSampler.js`
- *Action* : implémenter les hooks. Démotion L1→L2 : `pool.couperAvecFondu(voix, FONDU)` (réutilise `_cut`, ligne 153) puis `sectors.absorberImpact(voix.pos, material, head)` ; émettre `crossfade` pendant la zone. Promotion L2→L1 : si `pool.free.length` → `pool.play(...)`, sinon émettre `lod{reason:'no-budget'}` et rester en densité.
- *Dépend* : T-3.2, T-2.4
- *Test* : traverser `r1` lentement → events `lod`+`crossfade`, **aucun** `steal.remaining>0` lié à la démotion.

**T-3.4 — Évaluation périodique** · `ds/ui_kits/diorama/DioramaApp.jsx:182` (boucle existante)
- *Action* : à 30 Hz, mettre à jour `s.dist` des sources suivies puis `lodController.évaluerLod(...)`. Ne suivre que les voix dont `dist ∈ [r−overlap−h, r+overlap+h]`.
- *Dépend* : T-3.3
- *Test* : pas de papillonnement (`lod` A→B→A < 300 ms ≈ 0) sur un déplacement nominal.

**T-3.5 — LOD levier de budget** · `ds/ui_kits/diorama/RainSampler.js` (`ajusterBudget`, ~1 Hz)
- *Action* : implémenter `ajusterBudget(stats, params, rec)` (§5) : abaisse `r1` de `PAS` si `busy ≥ busyHi·size` (borné à `R1_MIN`), relève si `busy < busyLo·size` (borné à `cfg.layers.L1.rMax`). Émet `budget`. Appelé par un `setInterval(…, 1000)` dans DioramaApp.
- *Dépend* : T-3.3
- *Test* : densité max → `r1` baisse, event `budget.r1Adj` décroît, `steals` se stabilise.
